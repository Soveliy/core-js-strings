...
function countVowels(str) {
  ...
  return str.split('').filter((sym) => vowels.includes(sym)).length;
}
...
function isPalindrome(str) {
  const currentStr = str.replaceAll(/\W/g, '').toLowerCase();
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    if (array[i].length > bigestWord.length) {
      bigestWord = array[i];
    }
  }
  return bigestWord;
...
function encodeToRot13(str) {
  ...
  const alphabetROT = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm ?!';
  return str
    .split('')
    .map((sym) => alphabetROT[alphabet.indexOf(sym)])
    .join('');
}
